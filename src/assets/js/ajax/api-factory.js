...
export class Descriptor {
    constructor(desc) {
        ...
        this._authorization = window.localStorage.getItem("token") ? window.localStorage.getItem("token") : 'Basic eWdnOnlnZw==';
    }
    ...
    makeRequest(method, params) {
        ...
        const headers = _.extend({
            ...
            'source': isWeiXin() ? 'weixin' : 'H5',
            // 'source': 'H5',
            'token': window.localStorage.getItem("token") ? window.localStorage.getItem("token") : 'Basic eWdnOnlnZw==',
        }, this._headers);
        return {
            data: data,
            method,
                ...
                ajax(ajaxOption)
                    .then(v => {
                        ...
                        if (v.data.code === 200) {
                            ...
                            // loginout登录会话超时
                            // return false;
                        } else if (v.data.code === 503) {
                            window.alert('刷新太快啦，请稍后再试')
                        } else {
                            // window.alert('其他')
                        }
