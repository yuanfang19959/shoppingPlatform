<template>
  <div class="main">
    ...
    <mt-popup v-model="popupVisible" popup-transition="popup-fade" position="bottom" class="popupShow">
      <vue-pickers :show="show" :defaultData="defaultData" :selectData="pickData" value-key="text" @cancel="close"
        @confirm="confirmFn">
      </vue-pickers>
    </mt-popup>
  ...
  methods: {
    ...
    confirmFn(val) {
      ...
      if (this.AreaType == 0) {
        ...
        if (
          val.select1.value != this.CSelected.value &&
          val.select1.value != this.PSelected.value
        ) {
        ...
        if (
          val.select1.value != this.DSelected.value &&
          val.select1.value != this.CSelected.value &&
          val.select1.value != this.PSelected.value
        ) {
    ...
    submitForm() {
      let phone = this.form.phone,
        addressSupple = this.form.addressSupple;
      if (!phone) {
        Toast("手机号码不能为空");
        return;
      } else {
