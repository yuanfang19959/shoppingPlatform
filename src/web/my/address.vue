<template>
  <div class="main">
    ...
    <!--<div class="content" v-for='(i,index) in list' v-if='i.isDefault' :class="{activeContent2:i.isDefault}" @click="$router.push({path:'/confirmTheOrder',query:{addressId:i.id}})">-->
    ...
    <!--<div class="content" v-for='(i,index) in list' v-if='!i.isDefault' :class="{notactiveContent2:!i.isDefault}" @click="$router.push({path:'/confirmTheOrder',query:{addressId:i.id}})">-->
      <div class="c1">
        <div class="left">
          <div class="top">
            <span class="s1">{{i.name}}</span>
...
export default {
  ...
  data() {
    return {
      barType:'0',  
      barText:{
        leftData:{type:2,name:require("@/assets/imagea/blackup.svg"),path:'/my/index'},
  ...
  mounted () {
    ...
    if(this.$route.query.pageType){
      ...
      if(this.pageType==1){
        ...
        this.barText.leftData.path = "/orderDetails?id="+this.$route.query.orderId;
      }else{
  ...
  methods: {
    ...
    // 判断跳转其他页面 
    goPageOther(item){
      let url,type=this.pageType;
      
      if(type==1){
        this.$router.push({path:'/confirmTheOrder',query:{addressId:item.id}});
      }else if(type==2){
        if(this.$route.query.orderId){
          let orderId=this.$route.query.orderId;
          url='/orderDetails?id='+orderId;
          this.$router.push({path:url,query:{OrderAddrData:JSON.stringify(item)}});
        }
      }
    },
    ...
    edit(id){
      //编辑带pageType=1
      if(this.$route.query.pageType){
          this.$router.push({path:'/my/addressEdit',query:{addressId:id, pageType:1}});
      }else{
          this.$router.push({path:'/my/addressEdit',query:{addressId:id }});
      }  
    }
