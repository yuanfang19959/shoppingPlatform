<template>
  <div class="main">
      ...
      <div class="box" v-for="item in goodsData.listDetail" :key="item.spid">
        ...
        <div class="goods">
          ...
          <div class="rightInfo">
            ...
            <!--<span class="kg" v-if="item.weight">净含量{{item.weight }}千克</span>-->
          </div>
        ...
        <div class="song">
          ...
          <!--<div class="song_top">
            <span class="p1">优惠</span>
            <span class="p2">-¥10.00</span>
          </div>-->
          <div class="song_middle">
            ...
            <!--<span class="">¥{{goodsData.orderMoney|goodsTotalMoneyFilter(goodsData.shippingFee)}}</span>-->
          </div>
          <div class="song_bottom">
            ...
            <!--<span class="p2">¥{{goodsData.orderMoney }}</span>-->
          </div>
        </div>
    ...
    <!-- 商品退还 -->
    <!-- <div class="wl" v-show="goodsData.status==2 || goodsData.status==3 || goodsData.status==1"> -->
        <!-- 上面v-show中新加的内容 限定用户在待评价和待收货的时候才能进行商品退换非退款 -->
      <!-- <div class="wl_center" @click="exChange(2)" >
        <span>商品退换</span>
      </div>
      <div class="wl_right">
        <img :src="leftarrow" alt="" />
            ...
            <li @click='isShow=!isShow' class="moreItem" v-show="goodsData.status==1||goodsData.status==2||goodsData.status==3">
              ...
              <div class="morePopup" v-show='isShow'>
                <ul class="morePopupList">
                  ...
                  <!-- 这边加v-if="this.isExchange" 判断是否支持退换货 -->
                  <!--<li class="triangle_topright"></li>-->
                </ul>
...
export default {
  ...
  data() {
    return {
      barType:'0',  
      barText:{
        leftData:{type:2,name:require("@/assets/imagea/blackup.svg"),path:"/order"},
  ...
  methods: {
    ...
    getpagelist(id) {
      ...
      extendOrder(type) {
          // -1 删除订单 0 修改地址 1 取消订单 2 付款 3 待发货  5 确认收货 6 延长收货  7 换货中 8 退货中 
          let url, data, orderId = parseInt(this.goodsData.id);
          if (type == -1) {
            url = 'order/deleteOrderinfo';
            data = {id:orderId};
          } else if (type == 0) {
            this.$router.push({path:"/my/address",query:{pageType:2,orderId:orderId}});
            // return;
          } else if (type == 1) {
            // url='order/cancelOrderinfo';
            // url='order/cancelOrderinfo?id=78';
      ...
      exChange(type){
        ...
        console.log(this.isExchange)
          if(this.isExchange){
              console.log(this.isExchange)
              if(type==1){ 
                 this.$router.push({path:"/applyRefund",query:{id:this.$route.query.id}}); 
              }else{
                 this.$router.push({path:'/exchangeGoods',query:{id:this.$route.query.id}})
              }
  ...
  filters: {
    ...
    orderTotalNumFilter(goodnum,num,fee){
      let Tgoodnum=parseFloat(goodnum),Tnum=parseFloat(num),Tfee=fee;
      return Tgoodnum*Tnum+Tfee;
    },
    // 商品总价
    goodsTotalMoneyFilter(goodnum,num){
      console.log("goodnum==",goodnum,num);
      let Tgoodnum=parseFloat(goodnum),Tnum=parseFloat(num);
