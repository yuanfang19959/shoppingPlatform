  ...
  <div class="comfirmBox" v-if="confirmBox.boxisShow">
    ...
    <div class="test" v-if="confirmBox.isShow">
      ...
      <div class="messageBxo">
        ...
        <div class="title">
          支付信息展示费
          <img src="~@/assets/images/close.png" alt="" class="close" @click="confirmBox.isShow = false;confirmBox.boxisShow = false;confirmBox.transPassword=''">
        </div>
        <div class="realTitle">
          实际支付
        </div>
        <div class="amount">
          ¥{{confirmBox.amount | formatMoney}}元
        </div>
        <div class="payStyle" @click="getTable()">
          <div class="style">支付方式</div>
          <div class="name" v-if="confirmBox.payType === 3"  @click="confirmBox.isShow = false;payStyleBox.isShow = true;">{{confirmBox.str}}尾号{{confirmBox.bankCardNo | bankCardShow}}<img src="~@/assets/images/back2_icon@2x.png" alt=""></div>
          <div class="name" v-if="confirmBox.payType === 1" @click="confirmBox.isShow = false;payStyleBox.isShow = true;">余额支付(可用：¥{{balance}})<img src="~@/assets/images/back2_icon@2x.png" alt="" srcset=""></div>
        </div>
        <!-- <div class="valideCode" v-if="confirmBox.payType === 2">
          <div class="style">验证码</div>
          <input name="carType" maxlength="6" v-model="confirmBox.validatecode" type="text" class="input_text" placeholder="请输入验证码" />
          <div class="reSendMsg" v-show="!isShow" @click="getCode()">获取</div>
          <div class="reSendMsg countDown" v-show="isShow">{{count}} s</div>
        </div> -->
        <input type="password" v-if="confirmBox.payType === 1" maxlength="6" class="pwd" placeholder="请输入交易密码" v-model="confirmBox.transPassword">
    ...
    <div class="test" v-if="payStyleBox.isShow">
      ...
      <div class="messageBxo">
        ...
        <div class="title">
          支付方式
          <img src="~@/assets/images/back_icon@2x.png" width="15" height="23" alt="" class="close" @click="payStyleBox.isShow = false;confirmBox.isShow = true;">
        </div>
...
export default {
  ...
  methods: {
    ...
    getCode() {
      ...
      ajax({
        url: 'order_loans/info_pay',
        optionParams: {
          userCardId: this.confirmBox.userCardId,
          loansId: this.confirmBox.loansId,
    ...
    confirmZhiFu() {
      ...
      // if(this.confirmBox.isUndertake==false){
        ajax({
          url: 'order_loans/info_pay',
          optionParams: {
            userCardId: this.confirmBox.userCardId,
            ...
            payType: this.confirmBox.payType,
            transPassword: this.confirmBox.transPassword,
            callback:'http://192.168.0.32:8085/#/my/myoweundertake'
          }
        }).post()
          .then(res => {
            if (res.code === 200) {
              ...
              this.$router.push({ path: '/my/myoweundertake', query: { id: this.confirmBox.loansId,wxshare:false} })
      ...
      // if(this.confirmBox.isUndertake==true){
      //   ajax({
      ...
      //         window.localStorage.removeItem("oweDebt");
      //         window.localStorage.setItem("title", this.confirmBox.pfName);
      //         this.$router.push({ path: '/my/myoweundertake', query: { id: this.confirmBox.loansId,wxshare:true} })
      ...
      ajax({
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            // window.localStorage.removeItem("oweDebt");
            // this.$router.push({ path: '/my/detailsPendingPay', query: { id: this.confirmBox.transOrderId } });
    ...
    //确认支付按钮
    confirmCash() {
      console.log(this.confirmBox.payType)
      ...
      if (this.confirmBox.payType === 1) {
        if (!this.confirmBox.transPassword || this.confirmBox.transPassword.length != 6) {
          Toast('请输入6位交易密码!');
          return false;
        }
